...
export async function runCycle(input: CycleInput): Promise<CycleOutput> {
  ...
  // Deadline = cycleTimeoutMs + 60s cleanup buffer. If the spawned
  ...
  // long gap between queued and dequeued (queue head stuck) vs a long
  // gap between dequeued and any subsequent cycle.stage line (cycle
  // body stuck somewhere specific).
  let queueLog: Logger | null = null;
  try { queueLog = new Logger(input.profile); } catch { /* missing profile */ }
  const queuedAt = Date.now();
  queueLog?.info("cycle.queued", { source: input.task ? "explicit" : "heartbeat" });

  return serialize(input.profile, () => {
    queueLog?.info("cycle.dequeued", { waitMs: Date.now() - queuedAt });
    return runCycleInner(input);
  }, deadlineMs);
}
...
async function runCycleInner(input: CycleInput): Promise<CycleOutput> {
  ...
  try {
    ...
    log.info("cycle.stage", {
      stage: "skills-index.done",
      count: skillsIndex.skills.length,
      hash: skillsIndex.manifestHash.slice(0, 8),
    ...
    if (narrateEnabled) {
      ...
      if (editableSource && source) {
        ...
        log.info("cycle.stage", { stage: "narrator-open.start", source });
        progressHandle = await openProgressMessage(
          input.profile,
          source,
          "🔄 working…",
        );
        log.info("cycle.stage", { stage: "narrator-open.done", ms: Date.now() - openStart, ok: progressHandle !== null });
        if (progressHandle) {
          const srcRef = source;
      ...
      // The narrator picks up skill_view tool_use events from the stream;
      // we no longer pre-seed it with matched skills since the index
      // pattern advertises everything available, not what's pre-selected.
    }
    ...
    log.info("cycle.stage", { stage: "backend.start", mode: useStream ? "stream" : "once", model: picked.model, effort: cfg.effort });
    ...
    log.info("cycle.stage", { stage: "backend.done", ms: Date.now() - backendStart, ok: result.ok, turns: result.turns });
    ...
    // Post-cycle memory extraction. Skip on cheap (Haiku) cycles and on
    // trivially short responses to keep token usage tight. Skill
    // self-learning happens mid-cycle via skill_manage MCP tool, not here.
...
export function assemblePrompt(input: AssembleInput): string {
  ...
  if (input.skillsIndex.trim()) sections.push(`# Skills\n${input.skillsIndex.trim()}`);
