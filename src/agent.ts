...
async function runCycleInner(input: CycleInput): Promise<CycleOutput> {
  ...
  try {
    ...
    log.info("cycle.stage", { stage: "skill-match.start", count: allSkills.length });
    const matched = await matchSkills(allSkills, task, budget.skillCount, {
      allowedTools: cfg.allowedTools,
      strategy: cfg.skillMatcher ?? "llm",
      onTrace: (route, names, err) => log.info("skill.match", { route, names, ...(err ? { err } : {}) }),
    });
    log.info("cycle.stage", { stage: "skill-match.done", ms: Date.now() - matchStart, picked: matched.length });
