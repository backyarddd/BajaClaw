...
function scanDir(dir: string, scope: SkillScope): Skill[] {
  ...
  for (const entry of entries) {
    ...
    // Accept SKILL.md (bajaclaw + openclaw canonical) or the
...
export function parseSkill(raw: string, path: string, scope: SkillScope): Skill | null {
  ...
  const bajaclawMeta = firstObject(meta, ["bajaclaw"]);
  const origin: SkillOrigin = openclawMeta ? "openclaw" : "bajaclaw";

  // Platform list. Top-level `platforms` (kebab-format) or
  ...
  // Conditional activation under metadata.bajaclaw.
  const requiresTools = arrField(bajaclawMeta?.requires_tools);
  const requiresToolsets = arrField(bajaclawMeta?.requires_toolsets);
  const fallbackForTools = arrField(bajaclawMeta?.fallback_for_tools);
  const fallbackForToolsets = arrField(bajaclawMeta?.fallback_for_toolsets);

  // Presentation / pointers.
  const tags = arrField(bajaclawMeta?.tags);
  const related = arrField(bajaclawMeta?.related_skills);
  ...
  // fall back to tags so the matcher has something to score on.
  const triggers = arrField(fm.triggers) ?? tags;

  const autoGenerated = fm.auto_generated === true || fm.auto_generated === "true" ? true : undefined;
