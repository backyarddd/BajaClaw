...
// format as the lingua franca; each adapter translates as needed and yields
// text deltas. Streaming throughout; collect for non-stream callers.
...
async function* parseOpenAiSse(res) {
...
async function* codex(provider, messages, { model, signal }) {
  ...
  for (;;) {
    ...
    for (const line of parts) {
      ...
      try {
        const j = JSON.parse(t.slice(5).trim());
        if (j.type === "response.output_text.delta" && j.delta) yield j.delta;
      } catch { /* ignore */ }
    }
  }
}
...
// Public: full (non-stream) completion text.
export async function chat(messages, opts = {}) {
  let out = "";
  for await (const d of streamChat(messages, opts)) out += d;
  return out;
}
